...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Solidus client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Solidus"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Solidus"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
