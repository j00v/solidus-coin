...
const char * const PIVX_CONF_FILENAME = "solidus.conf";
const char * const PIVX_PID_FILENAME = "solidus.pid";
const char * const PIVX_MASTERNODE_CONF_FILENAME = "masternode.conf";


// Solidus only features
// Masternode
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\solidus
// Windows >= Vista: C:\Users\Username\AppData\Roaming\solidus
// Mac: ~/Library/Application Support/solidus
// Unix: ~/.solidus
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Solidus";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\solidusParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\solidusParams
    // Mac: ~/Library/Application Support/solidusParams
    // Unix: ~/.solidus-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SolidusParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "SolidusParams";
#else
    // Unix
    return pathRet / ".solidus-params";
#endif
#endif
}
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty solidus.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override solidus.conf
