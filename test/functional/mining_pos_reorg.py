...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected SLDS and zSLDS supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zSLDSsupply']:
                assert_equal(node['zSLDSsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check SLDS and zSLDS supply at the beginning
        # ------------------------------------------
        # zSLDS supply: 2 coins for each denomination
        ...
        # SLDS supply: block rewards minus burned fees for minting
        ...
        # Verify that SLDS and zSLDS supplies were properly updated after the spends and reorgs
        self.log.info("Check SLDS and zSLDS supply...")
